...
Missing data and accessibility enter the estimators on opposite sides of
the per-site formulas. On the numerator side, every diversity, divergence,
...
the fraction of called samples varies across sites. On the denominator
side, an optional \texttt{AccessibleMask} (a boolean array over genomic
...
\texttt{GenotypeMatrix} without separate function names. Missing data and
accessibility are handled automatically: the same call returns
calibrated rates whether the matrix is raw or has an attached
...
4-population statistics respectively; the gap to the aggregate
1{,}756$\times$ reflects that the aggregate uses the slower r-binned
\moments code path on $20\times$ more sequence, not a reconciliation
of the per-count numbers.
...
100{,}000 (simulated genotypes); for variant-count scaling
we held sample size at 200 haplotypes and varied \msprime simulated
...
subset of Ag1000G 3R (100 diploids / 200 phased haplotypes, 10.9M variants,
