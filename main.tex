...
median 139-fold and maximum 1{,}096-fold speedup; for
...
generalized $\theta$ framework of \citet{achaz2009} to efficiently derive all
...
$n_{\mathrm{hap}}\times n_{\mathrm{var}}$ \texttt{int8} array (with $-1$
...
\texttt{uint8} layout for kernels whose arithmetic naturally factorizes over
...
the matrix in a working \texttt{dtype} (typically \texttt{int8} or
\texttt{float32}), which can
...
for per-base normalization. \comment{To describe this a bit more, it
...
equivalent we observe agreement to either machine precision (Table~\ref{tab:accuracy}). 
...
sums under different reductions. A directly comparable Hudson
...
For sample-size scaling we held the variant count fixed at 100{,}000 and varied haplotypes from 100 to 100{,}000.
For variant-count scaling, we held sample size at 200 haplotypes and
varied the span of the simulated sequence length from $\sim 10^3$ to
...
structure deviates from the genome-wide pattern. As a sanity
...
the localized peak of Garud's $H_{12}$ at the sweep focal site.
...
streaming approach keeps only one window's matrix in memory at a
time instead of storing all $\sim10{,}900$ window matrices at
once. Three distinct corner clusters are recovered, and the
corner-1 region near $\sim$28\,Mb overlaps with a localized
...
\pggpu-parsed LD statistics inside the standard \moments optimizer
