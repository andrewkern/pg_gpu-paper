...
\affil[4]{\small{Oregon Health \& Science University, Division of Genetics}}
...
figures (including the per-population moments LD-parsing breakdown) are
...
without any tuning by the user. The biggest speedups are for two-locus
(linkage-disequilibrium) statistics, where the cost grows
quadratically with the number of variant sites; the same kernels also
back a drop-in replacement for the \moments-LD parser
\citep{ragsdale2019} and a GPU version of local PCA / \lostruct
\citep{li2019}. Full details---the array layouts, the fused-kernel
families, how missing data and accessibility masks are handled, the
chunking rule, and the downstream interfaces---are in the
Supplementary Material (Extended Design and Implementation).
...
\texttt{windowed\_analysis}, the fused-kernel entry point for genome
scans (Design and Implementation). A complete listing of all
...
We first verified that \pggpu reproduces established CPU
...
curve overlay to within line width between \scikitallel and \pggpu while
...
the Ag1000G gains are not artifacts of the empirical dataset.
...
single-import drop-in replacement, with parameter estimates
indistinguishable from the native parser---is described in the
...
tuning. Third, processing windows in tiles---so that working memory
...
diploid genotypes as an $n_{\mathrm{var}}\times n_{\mathrm{samp}}$
...
(\texttt{moments.LD.Parsing.bootstrap\_data}, \texttt{optimize\_log\_lbfgsb},
