...
LD-decay computation run $5.8\times$ and $254\times$ faster
(Figure~\ref{fig:overview}); a controlled cross-check on simulated
...
Multi-population LD parsing for demographic inference is a separate
bottleneck \citep{ragsdale2019}. On a four-population coalescent
...
speedup (Figure~\ref{fig:moments_ld})---making whole-genome LD-based
inference tractable on standard hardware; a per-population-count
breakdown is given in Supplementary Figure~\ref{fig:moments_ld_breakdown}.
...
realistic LD; Figure~\ref{fig:scaling}). Per-site statistics scale
linearly on both axes and finish in well under a second across the
tested range; two-locus statistics show the expected $O(n^2)$ growth
with sample size for Garud's $H$ and a sub-quadratic profile for
$Z_{nS}$ (a sampled-pairs estimator). At the largest configuration
tested (100{,}000 haplotypes $\times$ 100{,}000 variants) every
single-pass statistic completes in under ten seconds on one A100.
